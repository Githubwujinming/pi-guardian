...
export function registerRespondTool(pi: ExtensionAPI): void {
	pi.registerTool({
		...
		execute: async (
			...
			if (params.optionIndex != null) {
				...
				for (let i = 0; i < idx; i++) {
					...
					await pi.exec("herdr", ["pane", "send-keys", paneRef, "Down"], {
						signal,
					});
				}
				if (signal?.aborted) throw new Error("Canceled");
				await pi.exec("herdr", ["pane", "send-keys", paneRef, "Enter"], {
					signal,
				});

				return {
					content: [
						{ type: "text", text: `Selected option ${idx} in ${paneRef}` },
					],
			...
			if (params.options != null && params.options.length > 0) {
				...
				for (const target of sorted) {
					...
					currentPos = target;
					if (signal?.aborted) throw new Error("Canceled");
					await pi.exec("herdr", ["pane", "send-keys", paneRef, "Space"], {
						signal,
					});
				}
				...
				return {
					content: [
						{
							type: "text",
							text: `Toggled options [${sorted.join(", ")}] in ${paneRef}`,
						},
					],
			...
			if (params.text != null) {
				await pi.exec("herdr", ["pane", "run", paneRef, params.text], {
					signal,
				});

				return {
					content: [
						{ type: "text", text: `Sent "${params.text}" to ${paneRef}` },
					],
			...
			throw new Error(
				"respond requires one of: optionIndex, options[], or text",
			);
		},
		...
		renderCall: (args: RespondArgs, theme) => {
			...
			if (args.optionIndex != null)
				parts.push(`  optionIndex: ${args.optionIndex}`);
		...
		// -----------------------------------------------------------------------
		renderResult: (
			result: AgentToolResult<RespondDetails>,
