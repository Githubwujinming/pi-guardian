...
export const BUILTIN_PATTERNS: BuiltinPattern[] = [
  ...
  // Implementation complete/paused at Phase N — 实施完成
  {
    ...
    regex: /Implementation\s*(complete|paused)\s*(at|for)?\s*(Phase|phase)?\s*\d*/i,
  },
  ...
  // /skill:xxx .rpiv/artifacts/... — 链式技能调用
  {
    ...
    regex: /\/skill:(implement|plan|design|research|validate|commit|revise|blueprint)\s+\.rpiv\/artifacts/i,
  },
  ...
  {
    name: "question-end",
    regex: /(Shall\s+I|Would\s+you|Do\s+you|Can\s+I|Should\s+I|Could\s+you|Will\s+you).*\?/i,
  },

  // ===== ask_user_question 中文模式 =====
  {
    name: "chinese-question",
    regex: /(要不要|是不是|能不能|会不会|可不可以|是否|需不需要|有没有|可以.*吗|要.*吗|需要.*吗)/i,
  },
  {
    name: "chinese-choice",
  ...
  {
    name: "press-enter",
    regex: /[Pp]ress\s+(Enter|any\s+key)\s+to\s+(continue|proceed|confirm)|按.*回车|按下.*Enter/i,
  },
